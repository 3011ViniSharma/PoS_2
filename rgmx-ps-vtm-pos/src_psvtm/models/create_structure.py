...
def get_pair_summary_drop(input_data_flags_df: pd.DataFrame, params:dict) -> pd.DataFrame:
    ...
    input_df_good_grouped['general_flag'] = input_df_good_grouped['cumm_flag'].apply(
        ...
        lambda x: "DROP: " + str(drop_thresh) + " cut off" if x == 1 else "GOOD") #updated based on JK feedback during posvtm testing
...
def calc_opi_xpi(input_attr_df: pd.DataFrame, attr:str, params:dict, filt_index: pd.Series) -> pd.DataFrame:
    ...
    input_attr_df['combine'] = per.astype(str) + " " + input_attr_df[channel_col].astype(str) + item_u.astype(str) + " " + filt_index.astype(str)
...
def get_res_df(target:pd.DataFrame, input_df:pd.DataFrame,
               loess_sh:pd.DataFrame, dummies_df:pd.DataFrame, attr:str, params:dict,
    ...
    :param pd.DataFrame dummies_df: dummies to be used for defining seasonality variables
    ...
    seasons_col = list(dummies_df.columns)[0:(len(dummies_df.columns)-1)]
    ...
    for ui in unique_items:
        ...
        if non_zero_idx.sum() > 30:
            ...
            gross_elas = np.array(LinearRegression(normalize=True).fit(X_gross_elas, y_gross_elas).coef_[0])
...
def attr_imp(input_df: pd.DataFrame, attr:str, input_filt: pd.Series, params:dict = None, useshare:bool=True):
    ...
    #data_for_loess_sh = pd.get_dummies(data_for_loess_sh, columns=[seasons_var], prefix='', prefix_sep='') #add dummies for seasons
    data_for_loess_sh_w_dummies = pd.get_dummies(data_for_loess_sh, columns=[seasons_var], prefix='', prefix_sep='') #add dummies for seasons
    data_cols = list(data_for_loess_sh.columns)
    data_cols.remove(seasons_var)
    dummies = data_for_loess_sh_w_dummies.drop(data_cols,axis=1) # get dummies for seasons
    data_for_loess_sh = data_for_loess_sh_w_dummies #overwriting dataset with the one having dummies as well
    data_for_loess_sh = data_for_loess_sh.replace([np.inf, -np.inf], np.nan).fillna(0)
    catalog = CATALOG
    ...
    res_df = get_res_df(target=res_calc_df, input_df=input_df, loess_sh=loess_sh, attr=attr, params=params, useshare=True, filt_index=outlier_filt, dummies_df = dummies)
    if res_df is None:
        return [None, None]
    summary_res_df = get_agg_elasticities(res_df, attr=attr, capp=2, params=params)

    return [summary_res_df, res_df]
    ...
    for attr in filt_list:
        ...
        if att_df is None:
            ...
            if (sum_att_df == 0 and len(att_df) > 2):
                ...
                logger.info(f"Attribute does not qualify requirements for calculating indexes. Dropping.")
...
def get_attr_summary(input_df: pd.DataFrame, attrlist: list, input_filt:pd.Series=True,
    ...
    return [score_table, batch_attr_imp]
...
def ps_break(input_df: pd.DataFrame, attrlist:List,filter_index: pd.Series, purchase_structure: pd.DataFrame, current_level: int, params:Dict) -> List:
    ...
    attr_summ = get_attr_summary(input_df = input_df, attrlist=attrlistx,
                                 input_filt=filt_flag_index, size_validated=0.045, #Change Yash
                                 useshare=True, params=params)
...
def ps_break_between(input_df: pd.DataFrame, attrlist:List,filter_index: pd.Series, purchase_structure: pd.DataFrame, current_level: int, params:Dict) -> List:
    ...
    #                             useshare=True, params=params)
    attr_summ = get_attr_summary(input_df = input_df, attrlist=attrlistx,
                                 input_filt=filt_flag_index, size_validated=0.045, #Changed Yash
                                 useshare=True, params=params)
    if attr_summ is None:
        logger.info("No valid attributes for this break")
        return [purchase_structure, "No valid attributes for this break"]
...
def create_structure_between(attr_df: pd.DataFrame, purchase_struct_df: pd.DataFrame, interim_result: pd.DataFrame,attrlist:list, filt_idx: pd.Series=[True],lvl:int=0,
    ...
    if purchase_struct_df is None or lvl <= 0: 
        ...
        temp_result = interim_result[interim_result['L']==filt_lvl][interim_result['_id'].astype(str).str.rstrip('-')==id_filter]
        ...
        print("forced_column_name",forced_column_name)
...
def get_inputs_force_PS(df_ps, df_ps_good, attrlist):
    """
    Description: get original PS dataframe and forces a change in tree structure based on user inputs
    
    Inputs: df_ps: dataframe used for purchase structure
    df_ps_good: dataframe used for purchase structure with good observations
    
    Outputs: df_ps with new purchase structure after forcing
    """
    
    #Levels to modify
    level_columns = [c for c in df_ps.columns.to_list() if c.startswith("X")]
    n = len(level_columns)
    
    level_selected = input("Which level you want to modify? Please select a number from 2 to "+str(n))
    check_level = level_selected in map(str,list(range(2,n+1)))
    
    while check_level is False:
        level_selected = input("Please write only a number from 2 to "+str(n))
        check_level = level_selected in map(str,list(range(2,n+1)))
        
    #Rows to modify
    levels_previous = ['X' + s for s in map(str,list(range(1,int(level_selected))))]
    df_ps_f = df_ps.copy()
    
    cond = 1
    for l in levels_previous:
        valid_values = df_ps_f[l].unique()
        print("Please select a value for level "+l+". Valid options are:")
        print(valid_values)
        value_selected = input()
        
        check_values = value_selected in valid_values
        while check_values is False:
            value_selected = input("Please enter only a valid option.")
            check_values = value_selected in valid_values
            #TODO: Value should not be last leaf 
        
        cond = cond & (df_ps_f[l]==value_selected)
        df_ps_f = df_ps_f.loc[(df_ps_f[l]==value_selected),:].copy()
        
    #Values to modify
    #df_ps_good[attrlist].select_dtypes(include=['object']).columns
    str_columns = df_ps_good[attrlist].select_dtypes(include=['object']).columns
    valid_columns = []
    
    for c in str_columns:
        n_values = df_ps_good[c].value_counts().size
        if  n_values >1 & n_values <= 3:
            valid_columns = valid_columns + [c]

    #Replace
    print("Please select the column. Valid options are:")
    print(valid_columns)
    selected_column = input()
        
    df_ps.loc[df_ps_f.index,'X'+level_selected] = df_ps_good.loc[df_ps_f.index,selected_column]
    
    print("Structure replaced.")
    
    return df_ps, int(level_selected), cond